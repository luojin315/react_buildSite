...
export default class control_suspendedWindow extends Component {
    ...
    closeWindow = () => {
        //关闭悬浮窗 此组件
        this.props.openWindow(false)();
    }
    render() {
        const { head } = this.state;
        const { chengeState } = this.props;
        return (
                <div data-show={chengeState} className="control_suspendedWindow" key="window" style={{ top: '100px', left: '200px', display: chengeState ? 'block' : 'none' }}>
